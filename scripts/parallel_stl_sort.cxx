...
template <typename T> void print_front(const std::vector<T> &v) {
...
int main() {
  ...
  workVec.reserve(size);

  std::mt19937 engine;
  std::uniform_int_distribution<size_type> uniformDist(
      0, std::numeric_limits<size_type>::max());
  for (auto i = 0; i < size; ++i)
    randValues.push_back(uniformDist(engine));

  getExecutionTime(
      "std::copy() / std::sort() / std::unique() / std::execution::seq",
      [&randValues, &workVec]() {
        auto policy = std::execution::seq;
        workVec.resize(randValues.size());
        std::copy(policy, randValues.begin(), randValues.end(),
                  workVec.begin());
        print_front(workVec);
  ...
  getExecutionTime(
      "std::copy() / std::sort() / std::unique() / std::execution::par",
      [&randValues, &workVec]() {
  ...
  getExecutionTime(
      "std::copy() / std::sort() / std::unique() / std::execution::par_unseq",
      [&randValues, &workVec]() {
